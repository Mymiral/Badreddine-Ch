  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Convert to base64 for preview and fallback
        ...
        try {
          const fileName = `${Date.now()}_${user?.uid || 'guest'}.webm`;
          const { data, error } = await supabase.storage
            .from('audio')
            .upload(fileName, blob, { contentType: 'audio/webm' });
            
          if (error) {
            console.warn("Could not upload to Supabase storage, using base64 fallback.", error);
          } else {
            // Get public URL
            ...
            if (publicUrlData?.publicUrl) {
              setFormData(prev => ({ ...prev, audio: publicUrlData.publicUrl }));
            }
          }
        } catch (storageErr) {
          console.warn("Storage upload failed, using local base64.", storageErr);
        }
